...
class selectplay(Screen):
    ...
    def msgdeleteBouquets(self):
        self.session.openWithCallback(
            self.deleteBouquets,
            MessageBox,
            _("Remove all APSATTV Favorite Bouquet?"),
            MessageBox.TYPE_YESNO,
            timeout=5,
            default=True)
...
class main2(Screen):
    ...
    def updateMenuList(self):
        ...
        try:
            ...
            match = findall(
                regex_with_logo,
                content,
                DOTALL) if 'tvg-logo' in content else findall(
                regex_without_logo,
                content,
                DOTALL)
    ...
    def convert_bouquet(self, service):
        ...
        bouquet_name = 'userbouquet.astv_%s.%s' % (
                ...
                if isfile('/etc/enigma2/bouquets.tv'):
                    for line in open('/etc/enigma2/bouquets.tv'):
                        if bouquet_name in line:
                            in_bouquets = 1
                    if in_bouquets == 0:
                        if isfile('%s%s' % (enigma_path, bouquet_name)) and isfile(
                                '/etc/enigma2/bouquets.tv'):
                            Utils.remove_line(
                                '/etc/enigma2/bouquets.tv', bouquet_name)
